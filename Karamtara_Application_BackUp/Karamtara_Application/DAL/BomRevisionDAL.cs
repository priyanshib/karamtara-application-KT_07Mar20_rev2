...
namespace Karamtara_Application.DAL
{
    public class BomRevisionDAL
    {
        ...
        ExcelFill fill;
        Border border;
        public byte[] GetExcel(List<MasterModel> masterModels, int enqId)
        {
            connection = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand();
            DataSet ds = new DataSet();
            ...
            try
            {
                cmd = new SqlCommand("sp_getProjectName", connection);
                cmd.Parameters.Add(new SqlParameter("@enqId", enqId));
                cmd.CommandType = CommandType.StoredProcedure;
                adapter.SelectCommand = cmd;
                connection.Open();
                ...
                if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    ProjectName = Convert.ToString(ds.Tables[0].Rows[0]["Name"]);
                    EndCustomerName = Convert.ToString(ds.Tables[0].Rows[0]["EndCustomerName"]);
                    CustomerName = Convert.ToString(ds.Tables[0].Rows[0]["CustName"]);
                }
            }
            catch (Exception)
            {
        ...
        public int AddNewComponent(int bomId, int revNo, int prodId, int prodType, string tNumber)
        {
            try
            {
                ...
                bomMasterStatus += createBomDal.InsertData_BomMaster(prodId, prodType, bomId, revNo, tNumber);
                updateTableStatus += createBomDal.UpdateBOMTables(prodId, prodType, bomId, revNo);
                return 1;
            }
            catch (Exception)
            {
