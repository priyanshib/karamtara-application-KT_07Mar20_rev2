...
namespace Karamtara_Application.Controllers
{
    ...
    public class BOMRevisionController : BaseController
    {
        ...
        public ActionResult ViewBOM(int enqId, int bomId, int revNo)
        {
            ...
            bomModel.IsTenderUser = userInfo.UserType.ToLower() == "tender" || userInfo.UserType.ToLower() == "tender admin" ? true : false;
            bomModel.IsEdit = false;

            //return Json(bomModel);
        ...
        [HttpGet]
        public ActionResult EditBOM(int enqId, int bomId, int revNo)
        {
            if (!UserExist())
                return RedirectToAction("Index", "Login");
        ...
        [HttpGet]
        public ActionResult ViewTenderBOM(int enqId, int bomId, int revNo, int tndNo, int tndRevNo)
        {
            if (!UserExist())
                return RedirectToAction("Index", "Login");
            ...
            var userInfo = (UserModel)Session["UserData"];
            bomModel = bomRevDAL.GetBomProjDetails(bomId, revNo, enqId);
            bomModel.MasterList = bomRevDAL.ViewTenderBomData(bomId, revNo, enqId, tndNo, tndRevNo);
            bomModel.Summary = bomRevDAL.GetSummary(bomId, revNo);
            bomModel.BomId = bomId;
            bomModel.RevisionNo = revNo;
            bomModel.EnquiryId = enqId;
            ...
            return View(@"~\Views\Shared\Tender\BomCosting\ViewBOM.cshtml", bomModel);
        }

        [HttpGet]
        public ActionResult GetBomAuditTrial(int bomId)
        {
            if (!UserExist())
                return RedirectToAction("Index", "Login");

            bomRevDAL = new BomRevisionDAL();
            var auditModel = bomRevDAL.GetBomAudit(bomId);

            return PartialView("~/Views/Shared/BomAudit/_AuditTrial.cshtml", auditModel);
        }
        ...
        public ActionResult DownloadExcel(int bomId, int revNo, int enqId)
        {
            bomRevDAL = new BomRevisionDAL();
            Response.Clear();
            Response.BinaryWrite(bomRevDAL.GetExcel(GetDownloadExcel(bomId, revNo, enqId), enqId, bomId, revNo));
        ...
        [HttpGet]
        public ActionResult RemoveProds_Pg(int prodId, int prodType, int bomId, int revNo, int pgId, int assmId, int subAssmId)
        {
            bomRevModel = new BOMRevisionModel();
            bomRevDAL = new BomRevisionDAL();
            int status = 0;
            status = bomRevDAL.RemoveProdGrpData(prodId, prodType, bomId, revNo, pgId, assmId, subAssmId);
            return Json(status, JsonRequestBehavior.AllowGet);

        }

        [HttpPost]
        public ActionResult SaveBOMChanges(FormCollection form)
        {
            bomRevModel = new BOMRevisionModel();
            bomRevDAL = new BomRevisionDAL();
            int status = 0;
            status = bomRevDAL.SaveBOMChanges(form);
            return Json(status);
        }
