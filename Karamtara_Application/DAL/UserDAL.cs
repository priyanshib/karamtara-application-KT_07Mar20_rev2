    ...
    public class UserDAL
    {
        ...
        public void SendEmail(List<string> recievers, List<string> signupCodes)
        {
            ...
            for (int i = 0; i < recievers.Count; i++)
            {
                ...
                string emailBody = PopulateBody(recievers[i], signupCodes[i]);
        ...
        private string PopulateBody(string reciever, string signupCode)
        {
            string body = string.Empty;
            using (StreamReader reader = new StreamReader(HostingEnvironment.MapPath("~/EmailTemplates/acctCreation.html")))
            {
                body = reader.ReadToEnd();
            }
            body = body.Replace("{emailIdHere}", reciever);
            body = body.Replace("{signupCodeHere}", signupCode);
            return body;
        }

        public int ChangeUserActivation(int userId, bool IsEnabled)
        {
            connection = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand();
            cmd = new SqlCommand("sp_ChangeUserActivation", connection);
