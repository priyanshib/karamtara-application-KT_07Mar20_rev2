    ...
    public class SubAssemblyDAL
    {
        ...
        public SubAssemblyMasterModel GetComponentDetails(int subAssId)
        {
            ...
            try
            {
                ...
                if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    subAssmModel.Unit = Convert.ToString(ds.Tables[0].Rows[0]["Unit"]);
                    subAssmModel.SubUnitId = Convert.ToInt32(ds.Tables[0].Rows[0]["UnitId"]);
                }
                if (ds.Tables[1] != null)
                {
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
        ...
        public int EditSubAssembly(FormCollection form)
        {
            ...
            try
            {
                ...
                string CatNo = form["CatalogueNo"];
                string SubAssName = form["SubAssemblyName"];
                string UnitId = form["SubUnitId"];


                //Update Component Details
                if (componentId != null && Qty != null)
                {
                    connection = new SqlConnection(connectionString);
                ...
                //Update SubAssembly Details
                cmd = new SqlCommand("sp_EditSubAssDetails", connection);
                cmd.Parameters.Add(new SqlParameter("@SubAssName", SubAssName));
                cmd.Parameters.Add(new SqlParameter("@SubAssmId", Convert.ToInt32(subAssmId)));
                cmd.Parameters.Add(new SqlParameter("@Unit", Convert.ToInt32(UnitId)));
                cmd.Parameters.Add(new SqlParameter("@CatNo", CatNo));
                cmd.CommandType = CommandType.StoredProcedure;
                SubStatus = Convert.ToInt32(cmd.ExecuteScalar());
        ...
        public int CreateSubAssembly(FormCollection form)
        {
            ...
            try
            {
                ...
                var unitValue = strUnit.Split(new string[] { pattern }, StringSplitOptions.None).ToList();
                unitValue = unitValue.Select(x => x = x.Replace("~!", "")).ToList();
                ...
                using (var trans = connection.BeginTransaction(IsolationLevel.Snapshot))
                {
                    try
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@Unit", Convert.ToInt32(unitValue.Where(x => !string.IsNullOrEmpty(x)).Select(y => y).FirstOrDefault() ?? "0")));
        ...
        public int CreateComponents(FormCollection form)
        {
            ...
            var units = form["UnitId"] ?? string.Empty;
            ...
            try
            {
                ...
                var unitList = units.Split(new string[] { "," }, StringSplitOptions.None).ToList();
                unitList = unitList.Select(x => x = x.Replace("~!", "")).ToList();
                ...
                for (int i = 0; i < compNames.Count(); i++)
                {
                    if (!string.IsNullOrEmpty(compNames[i]))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@Unit", string.IsNullOrEmpty(unitList.ElementAtOrDefault(i)) ? 1 : Convert.ToInt32(unitList[i])));
        ...
        public List<ComponentModel> getComponenetMaterData(out List<UnitMaster> unitMasters, string prefix)
        {
            connection = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand();
            ...
            SqlDataAdapter adapter = new SqlDataAdapter();
            List<ComponentModel> compList = new List<ComponentModel>();
            unitMasters = new List<UnitMaster>();
            try
            {
                ...
                if (ds.Tables[0] != null)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        compModel.Unit = Convert.ToString(ds.Tables[0].Rows[i]["Unit"]);
                        compList.Add(compModel);
                    }
                }
                ...
                if (ds.Tables[1] != null)
                {
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                    {
                        ...
                        uModel.Id = Convert.ToInt32(ds.Tables[1].Rows[i]["Id"]);
                        uModel.UnitName = Convert.ToString(ds.Tables[1].Rows[i]["Description"]);
                        unitMasters.Add(uModel);
                    }
                }
                return compList;
            }
        ...
        public int SubmitComponentMaster(ComponentModel model)
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add(new SqlParameter("@CatalogueNo", string.IsNullOrEmpty(model.CatalogueNo) ? string.Empty : Convert.ToString(model.CatalogueNo)));
                cmd.Parameters.Add(new SqlParameter("@ComponentName", string.IsNullOrEmpty(model.ComponentName) ? string.Empty : Convert.ToString(model.ComponentName)));
                ...
                cmd.Parameters.Add(new SqlParameter("@Unit", model.UnitId));
                cmd.CommandType = CommandType.StoredProcedure;
            ...
            try
            {
                ...
                if (ds.Tables[0] != null)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        compModel.GalvanizedMaterial = Convert.ToString(ds.Tables[0].Rows[i]["GalvanizedMaterial"]);
                        compModel.UnitId = Convert.ToInt32(ds.Tables[0].Rows[i]["Unit"] ?? 0);
                    }
                }
                if (ds.Tables[1] != null)
                {
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
                    {
                        UnitMaster umModel = new UnitMaster();
                        umModel.Id = Convert.ToInt32(ds.Tables[1].Rows[i]["Id"]);
                        umModel.UnitName = Convert.ToString(ds.Tables[1].Rows[i]["Description"]);
                        unitList.Add(umModel);
                    }
                }
                //compModel.Unit = unitList.Where(x => x.UnitName == compModel.Unit).Select(y => y.Id).FirstOrDefault();
                compModel.UnitList = unitList;
                return compModel;
            }
            catch (Exception ex)
            {
