...
namespace Karamtara_Application.DAL
{
    public class ProductGroupDAL
    {
        ...
        public ProductGroupModel GetProductGroupData()
        {
            ...
            List<BundleSpacing> bundleSpacingList = new List<BundleSpacing>();
            try
            {
                ...
                if (ds.Tables[6] != null)
                {
                    ...
                    {
                        BundleSpacing bundle = new BundleSpacing();
                        bundle.Id = Convert.ToInt32(ds.Tables[6].Rows[i]["Id"]);
                        bundle.Spacing = Convert.ToString(ds.Tables[6].Rows[i]["Spacing"]);
                        bundleSpacingList.Add(bundle);
                    }
                }
                ...
                prodGroupModel.BundleSpacingList = bundleSpacingList;
        ...
        public int CreateProductGroup(FormCollection form)
        {
            ...
            try
            {
                ...
                var lineVoltageId = form["txtLineVoltage"];
                ...
                var conductorId = form["txtConductor"];
                var bundleSpacing = form["txtBundleSpacing"] ?? "";
                var conductorName = form["ConductorName"] ?? "";
                ...
                //var quantity = form["Quantity"] ?? string.Empty ?? "1";
                ...
                cmd.Parameters.Add(new SqlParameter("@LineVoltage", lineVoltageId));
                ...
                cmd.Parameters.Add(new SqlParameter("@ConductorType", conductorId));
                cmd.Parameters.Add(new SqlParameter("@conductorName", conductorName));
                cmd.Parameters.Add(new SqlParameter("@bundleSpacing", bundleSpacing));
                connection.Open();
...
;
        ...
        public ProductGroupModel GetProductGroupHierarchyById(int groupId)
        {
            ...
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0 && ds.Tables[0] != null)
                {
                    ...
                    model.ConductorName = Convert.ToString(ds.Tables[0].Rows[0]["ConductorName"]);
                    model.BundleSpacing = Convert.ToString(ds.Tables[0].Rows[0]["BundleSpacing"]);
                }
                if (ds.Tables[1] != null)
                {
                    for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
        ...
        public List<AutoCompModel> GetConductorNames(string prefix)
        {
            connection = new SqlConnection(connectionString);
            SqlCommand cmd = new SqlCommand();
            ...
            DataSet ds = new DataSet();
            SqlDataAdapter adapter = new SqlDataAdapter();
            List<AutoCompModel> modelList = new List<AutoCompModel>();
            try
            {
                cmd = new SqlCommand("GetConductorNames", connection);
                ...
                if (ds.Tables[0] != null)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        AutoCompModel model = new AutoCompModel();
                        model.Id = Convert.ToInt32(ds.Tables[0].Rows[i]["Id"]);
                        model.Value= Convert.ToString(ds.Tables[0].Rows[i]["Name"]);
                        modelList.Add(model);
                    }
                }
